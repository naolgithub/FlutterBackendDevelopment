  ...
  for (String element in users) {
    ...
    for (var element1 in char2List) {
      if (characterList.contains(element1)) {
        hasValue = true;
      } else if (characterList.contains(element1) == false &&
          listToBeDisplay.contains(element) == true) {
        hasValue = false;
    ...
    if (context.read<InputSearchCubit>().state.length > element.length &&
        listToBeDisplay.contains(element)) {
      listToBeDisplay.remove(element);
    }
    ...
    if (hasValue == true &&
        listToBeDisplay.contains(element) == false &&
        context.read<InputSearchCubit>().state.length <= element.length) {
      listnew.add(element);
...
List getListOfUsersInMessagesList(BuildContext context) {
  ...
  if (context.read<GetUserDataCubit>().state["messagesList"] != "" &&
      context.read<GetUserDataCubit>().state["messagesList"] != null) {
    for (String element
        in (context.read<GetUserDataCubit>().state["messagesList"] as dynamic)
            .keys) {
