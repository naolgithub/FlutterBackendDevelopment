...
class _ChatState extends State<Chat> with AutomaticKeepAliveClientMixin {
                          ...
                          child: Builder(builder: (context) {
                            ...
                            if (context
                              ...
                              FirebaseDatabase.instance
                                  .ref("usersData/$currentUser")
                                  .child("receivedReadPointers/$receiverName")
                                  .set(context
                                      .read<ReceivedMessagesCubit>()
                                      .state
                                      .keys
                            ...
                            context
                                .read<InputIndexPointerCubit>()
                                .isInputIndexPointer(context
                                        .read<SentMessagesCubit>()
                                        .state
                                        .length -
                                    1);
                            context
                                .read<ReceivedIndexPointerCubit>()
                                .isReceivedIndexPointer(context
                                        .read<ReceivedMessagesCubit>()
                                        .state
                                        .length -
                                    1);
                            return ListView(
                              ...
                              children: listofWidgets(
                                  context, currentUser, receiverName),
                            );
                          }),
                        ),
                      );
                      ...
                      child: SizedBox(
                        ...
                        child: TextField(
                          ...
                          onChanged: (value) {
                            ...
                            context
                                .read<InputSearchCubit>()
                                .startInputing(inputMessage.text);
                          },
                          showCursor: true,
                          dragStartBehavior: DragStartBehavior.down,
                          ...
                          decoration: InputDecoration(
                              constraints:
                                  const BoxConstraints.expand(height: 500),
                          ...
                          onPressed: () {
                            ...
                            if (context.read<InputSearchCubit>().state != "") {
                              ...
                              _scrollController1.animateTo(
                                  ...
                                  curve: Curves.fastOutSlowIn);
                              sendtheMessage(
                                  context.read<InputSearchCubit>().state,
  ...
  sendtheMessage(String message, String sender, String receiver,
      String timeasMillisecondsepoch) async {
    FirebaseDatabase.instance
        .ref("usersData/$sender/messagesList/$receiver/")
        .update({timeasMillisecondsepoch: message});
    FirebaseDatabase.instance
        .ref("usersData/$receiver/receivedList/$sender/")
        .update({timeasMillisecondsepoch: message});
  }
  ...
  createMessagesList(BuildContext context, String userName) {
    ...
    FirebaseDatabase.instance
        .ref("usersData/$currentUser")
        .child("messagesList/$userName/")
        .set({"123": "Say hii to $userName"});
    FirebaseDatabase.instance
        .ref("usersData/$userName")
        .child("messagesList/$currentUser/")
        .set({"125": "Say hii to $currentUser"});
  }

  getMessagesList(
      BuildContext context, String currentUser, String receiver) async {
    await context
        .read<SentMessagesCubit>()
        .getSentMessagesList(currentUser, receiver);
    await context
        .read<ReceivedMessagesCubit>()
        .getSentMessagesList(receiver, currentUser);
  }
...
List<Widget> listofWidgets(BuildContext context, userName, receiverName) {
  int length = context.read<ReceivedMessagesCubit>().state.length +
      context.read<SentMessagesCubit>().state.length;
  List<Widget> list = List.generate(
    length,
    (index) {
      if (context.read<ReceivedIndexPointerCubit>().state >= 0 &&
          context.read<InputIndexPointerCubit>().state >= 0) {
        if (((context.read<ReceivedMessagesCubit>().state.keys.elementAt(
                context.read<ReceivedMessagesCubit>().state.length -
                    context.read<ReceivedIndexPointerCubit>().state -
                    1) <
            context.read<SentMessagesCubit>().state.keys.elementAt(
                context.read<SentMessagesCubit>().state.length -
                    context.read<InputIndexPointerCubit>().state -
                    1)))) {
          context.read<MessageCubit>().inputMessage(context
              .read<ReceivedMessagesCubit>()
              .state
              .values
              .elementAt(context.read<ReceivedMessagesCubit>().state.length -
                  context.read<ReceivedIndexPointerCubit>().state -
                  1));
          if (context.read<ReceivedMessagesCubit>().state.keys.elementAt(
                  context.read<ReceivedMessagesCubit>().state.length -
                      ...
                      1) >
              1000) {
            context.read<DateTimeCubit>().setDateTime(context
                .read<ReceivedMessagesCubit>()
                .state
                .keys
                .elementAt(context.read<ReceivedMessagesCubit>().state.length -
          ...
          context.read<ReceivedIndexPointerCubit>().isReceivedIndexPointer(
              context.read<ReceivedIndexPointerCubit>().state - 1);
        ...
        else if (context.read<SentMessagesCubit>().state.keys.elementAt(
                context.read<SentMessagesCubit>().state.length -
                    context.read<InputIndexPointerCubit>().state -
                    1) <=
            context.read<ReceivedMessagesCubit>().state.keys.elementAt(
                context.read<ReceivedMessagesCubit>().state.length -
                    context.read<ReceivedIndexPointerCubit>().state -
                    1)) {
          List Msg = context.read<SentMessagesCubit>().state.values.toList();
          context.read<MessageCubit>().inputMessage(Msg[
              context.read<SentMessagesCubit>().state.length -
                  context.read<InputIndexPointerCubit>().state -
                  1]);
          if (context.read<SentMessagesCubit>().state.keys.elementAt(
                  context.read<SentMessagesCubit>().state.length -
                      context.read<InputIndexPointerCubit>().state -
                      1) >
              1000) {
            context.read<DateTimeCubit>().setDateTime(context
                ...
                .state
                .keys
                .elementAt(context.read<SentMessagesCubit>().state.length -
                    context.read<InputIndexPointerCubit>().state -
                    1));
          }

          context.read<InputIndexPointerCubit>().isInputIndexPointer(
              context.read<InputIndexPointerCubit>().state - 1);
          context.read<IsReceivedCubit>().messageIsSentOne();
        }
      } else if (context.read<InputIndexPointerCubit>().state < 0 ||
          context.read<ReceivedIndexPointerCubit>().state < 0) {
        if (context.read<InputIndexPointerCubit>().state >= 0 &&
            context.read<ReceivedIndexPointerCubit>().state < 0) {
          context.read<MessageCubit>().inputMessage(context
              .read<SentMessagesCubit>()
              .state
          ...
          context.read<InputIndexPointerCubit>().isInputIndexPointer(
              context.read<InputIndexPointerCubit>().state - 1);
          context.read<IsReceivedCubit>().messageIsSentOne();
        } else if (context.read<ReceivedIndexPointerCubit>().state >= 0 &&
            context.read<InputIndexPointerCubit>().state < 0) {
          List Msg =
              context.read<ReceivedMessagesCubit>().state.values.toList();
          context.read<MessageCubit>().inputMessage(Msg[
              context.read<ReceivedMessagesCubit>().state.length -
                  context.read<ReceivedIndexPointerCubit>().state -
                  1]);
      ...
      context
          .read<DateCubit>()
          .checkDateChange(context.read<DateTimeCubit>().state, context);
            ...
            context.read<IsDateChangedCubit>().state
                ? Container(
                    child: Text(
                      ...
                      style: TextStyle(
                          color: colors1[0], fontWeight: FontWeight.bold),
                    ),
                  )
                : Container(),
            Row(
